...
It is currently bootstrapped from `sqlglot-rust`, with an added parity harness that compares Rust output against a local Python SQLGlot checkout. The implementation stays Rust; Python is used only as the test oracle while the port catches up.
...
This project is derived from `sqlglot-rust`, which is inspired by and derived from Python [SQLGlot](https://github.com/tobymao/sqlglot). Both upstream projects are MIT licensed.
