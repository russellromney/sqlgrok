...
fn test_mysql_create_table_table_primary_key_auto_increment_to_sqlite() {
    ...
    validate_with_dialect(
        "CREATE TABLE x (id TEXT AUTO_INCREMENT, PRIMARY KEY (id))",
        "CREATE TABLE x (id TEXT AUTOINCREMENT PRIMARY KEY)",
        Dialect::Mysql,
        Dialect::Sqlite,
    );
}
...
#[test]
fn test_postgres_identity_table_primary_key_to_sqlite() {
    validate_with_dialect(
        "CREATE TABLE x (id INT GENERATED ALWAYS AS IDENTITY NOT NULL, PRIMARY KEY (id))",
        "CREATE TABLE x (id INTEGER PRIMARY KEY AUTOINCREMENT)",
        Dialect::Postgres,
        Dialect::Sqlite,
    );
    validate_with_dialect(
        "CREATE TABLE x (id INT GENERATED BY DEFAULT AS IDENTITY NOT NULL)",
        "CREATE TABLE x (id INTEGER AUTOINCREMENT)",
        Dialect::Postgres,
        Dialect::Sqlite,
    );
}
